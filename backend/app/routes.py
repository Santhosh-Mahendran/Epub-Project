...
def get_all_books():
    try:
        ...
        books = Book.query.filter_by(status='live').all()
...
@book_bp.route('/reader/get_books_by_cat/<int:category_id>', methods=['GET'])
@jwt_required()
def get_books_by_catfor_reader(category_id):
    try:
        # Get publisher_id from JWT
        reader_id = get_jwt_identity()
        ...
        # Get all books for the given category
        books = Book.query.filter_by(category_id=category_id, status='live').all()
        # Serialize book data
        books_list = []
        for book in books:
            books_list.append({
